...
import { list, findById, create, update, remove, init } from './todo.model.js';
...
router.post('/todolist', [
  ...
  */
  ...
  try{
    ...
    if(result.isEmpty()){
      if(req.body.saveIP){ // "ip 저장"에 체크했을 경우 실제 클라이언트 IP 추출
        req.body.ip = req.ip;
      }
...
router.delete('/todolist/init', async function(req, res, next) {
  ...
  */
  try{
    if(req.body.pwd === process.env.TODO_INIT_PWD){
