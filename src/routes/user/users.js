...
router.post('/', [
  ...
  body('password').trim().isLength({ min: 4 }).withMessage('비밀번호는 4글자 이상 입력해야 합니다.'),
  body('name').trim().isLength({ min: 2 }).withMessage('이름은 2글자 이상 입력해야 합니다.'),
  ...
  */
  ...
  try {
    ...
    const item = await userService.signup(clientId, req.body);
...
router.get('/email', [
  ...
  */
  ...
  try {
    const clientId = getClientId(req);
    const user = await userModel.findBy(clientId, { email: req.query.email });
    if (user) {
...
router.get('/name', [
  ...
  */
  ...
  try {
    const clientId = getClientId(req);
    const user = await userModel.findBy(clientId, { name: req.query.name });
    if (user) {
...
router.post('/signup/oauth', [
  ...
  */
  ...
  try {
    const clientId = getClientId(req);
    const item = await userService.signupOAuth(clientId, req.body);
    res.status(201).json({ ok: 1, item });
  } catch (err) {
    next(err);
  }
...
router.post('/login/kakao', async function (req, res, next) {
  /*
    ...
    #swagger.description = `
      ...
      API 서버에서 발급받은 키를 사용해야 하기 때문에 카카오 로그인을 구현할 팀은 강사에게 문의하시기 바랍니다.<br>
      응답 데이터에 token 속성으로 JWT 기반의 Access Token과 Refresh Token을 반환합니다.<br>
  ...
  */
  ...
  try {
    ...
    let user = await userService.loginKakao(clientId, kakaoUserInfo.id);
    ...
    if (!user) {
      ...
      // 회원 가입
      await userService.signupKakao(clientId, userInfo);
      // 로그인
      user = await userService.loginKakao(clientId, kakaoUserInfo.id);
      user.isNew = true; // 회원 가입을 한 경우에는 isNew 속성을 true로 설정
    }
...
router.post('/login/with', async function (req, res, next) {
  ...
  */
  try {
    ...
    const user = await userService.loginOAuth(clientId, req.body.providerAccountId);
...
router.get('/:_id/bookmarks', async function (req, res, next) {
  ...
  */
  ...
  try {
    ...
    const item = await bookmarkModel.findByUser(clientId, user_id);
    res.json({ ok: 1, item });
...
router.get('/:_id/*', /*jwtAuth.auth('user'),*/ async function (req, res, next) {
  ...
  */
  ...
  try {
    ...
    logger.log(attr);
    const item = await userModel.findAttrById(clientId, Number(req.params._id), attr);
    if (item) {
      res.json({ ok: 1, item });
    } else {
      next(); // 404
  ...
  */
  ...
  try {
    ...
    if (req.user.type === 'admin' || _id === req.user._id) {
      ...
      const clientId = getClientId(req);
      const updated = await userService.update(clientId, _id, req.body);
      if (updated) {
...
router.get('/', [
  ...
  */
  ...
  try {
    ...
    const result = await userModel.find(clientId, { search, sortBy, page, limit });
