...
// 게시글 등록
router.post('/', jwtAuth.auth('user', true), [ // 익명 가능
// router.post('/', jwtAuth.auth('user'), [ // 로그인 사용자만 가능
  ...
  */
  ...
  try {
    ...
    const item = await postModel.create(clientId, { ...req.body, views: 0, user });
...
router.get('/', [
  ...
  */
  ...
  try {
    ...
    const result = await postModel.find(clientId, { type: req.query.type, userId: req.user?._id, search, sortBy, page, limit });
...
router.get('/users', [
  ...
  */
  ...
  try {
    ...
    const item = await postModel.find(clientId, { type: req.query.type, userId: req.user?._id, search, sortBy, page, limit });
    res.json({ ok: 1, ...item });
  } catch (err) {
    next(err);
  }
...
router.get('/:_id', jwtAuth.auth('user', true), async function (req, res, next) {
  ...
  */
  ...
  try {
    const clientId = getClientId(req);
    const _id = Number(req.params._id);
    const item = await postModel.findById(clientId, { _id, userId: req.user?._id, incView: true });
    if (item) {
  ...
  */
  ...
  try {
    ...
    if (post && (req.user.type === 'admin' || post.user._id == req.user._id)) {
      const updated = await postModel.update(clientId, _id, req.body);
...
router.delete('/:_id', jwtAuth.auth('user'), async function (req, res, next) {
  ...
  */
  ...
  try {
    ...
    const post = await postModel.findById(clientId, { _id, userId: req.user._id });
    if (post && (req.user.type === 'admin' || post?.user._id == req.user._id)) {
      await postModel.delete(clientId, _id);
      res.json({ ok: 1 });
    } else {
      next();
    }
...
router.get('/:_id/replies', [
  ...
  */
  ...
  try {
    ...
    const result = await postModel.findReplies(clientId, { _id: Number(req.params._id), page, limit, sortBy });
  ...
  */
  ...
  try {
    ...
    if (post && (req.user.type === 'admin' || reply?.user._id == req.user._id)) {
      const item = await postModel.updateReply(clientId, _id, reply_id, req.body);
      res.json({ ok: 1, item });
    } else {
      next();
    }
...
router.delete('/:_id/replies/:reply_id', jwtAuth.auth('user'), async (req, res, next) => {
  ...
  */
  ...
  try {
    ...
    const reply = _.find(post?.replies, { _id: reply_id });
    if (post && (req.user.type === 'admin' || reply?.user._id == req.user._id)) {
      await postModel.deleteReply(clientId, _id, reply_id);
      res.json({ ok: 1 });
    } else {
      next();
    }
