...
router.post('/', jwtAuth.auth('user'), [
  ...
  */
  ...
  try {
    ...
    const item = await reviewModel.create(clientId, review);
...
router.patch('/:_id', jwtAuth.auth('user'), [
  ...
  */
  ...
  try {
    ...
    const clientId = getClientId(req);
    const review = await reviewModel.findById(clientId, _id);

    if (review && (req.user.type === 'admin' || review.user._id == req.user._id)) {
      const updated = await reviewModel.update(clientId, _id, req.body);
...
router.delete('/:_id', jwtAuth.auth('user'), async function (req, res, next) {
  ...
  */
  ...
  try {
    ...
    const review = await reviewModel.findById(clientId, _id);
    if (review && (req.user.type === 'admin' || review?.user._id == req.user._id)) {
      await reviewModel.delete(clientId, _id);
      res.json({ ok: 1 });
    } else {
      next();
    }
