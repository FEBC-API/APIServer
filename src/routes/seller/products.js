...
router.get('/', [
  ...
  */
  ...
  try{
    ...
    const result = await ProductModel.findBy(clientId, { sellerId: req.user._id, search, sortBy, page, limit, userId: req.user._id });
    
    for(const item of result.item){
      const orders = await SellerOrderModel.findByProductId(clientId, item._id, req.user._id);
...
router.get('/:_id', [
  ...
  */
  try{
    ...
    const item = await ProductModel.findById(clientId, { _id, seller_id });
    if(item){
      item.orders = await SellerOrderModel.findByProductId(clientId, _id, seller_id);
    }
...
router.post('/', [
  ...
  */
  ...
  try{
    ...
    const item = await SellerProductModel.create(clientId, newProduct);
...
router.patch('/:_id', [
  ...
  */
  ...
  try{
    const clientId = getClientId(req);
    const _id = Number(req.params._id);
    const product = await SellerProductModel.findAttrById(clientId, { _id, attr: 'seller_id', seller_id: req.user._id });
    if(req.user.type === 'admin' || product?.seller_id == req.user._id){
      const result = await SellerProductModel.update(clientId, _id, req.body);
...
router.delete('/:_id', async function(req, res, next) {
  ...
  */
  ...
  try{
    ...
    const product = await SellerProductModel.findAttrById(clientId, { _id, attr: 'seller_id', seller_id: req.user._id });
    if(req.user.type === 'admin' || product?.seller_id == req.user._id){
      const result = await SellerProductModel.delete(clientId, _id);
      res.json({ ok: 1 });
    }else{
      next(); // 404
    }
